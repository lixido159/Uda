...
public class MonthPickerScrollView extends View {
    ...
    private MonthChangeCallBack mMonthCallBack;
    ...
    public MonthPickerScrollView(Context context) {
        super(context);
        init();
    ...
    @RequiresApi(api = Build.VERSION_CODES.M)
    public MonthPickerScrollView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    @RequiresApi(api = Build.VERSION_CODES.M)
    public MonthPickerScrollView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void drawData(Canvas canvas){
        ...
        canvas.drawText(mData.get(mSelected)+"月",x,y,mPaint);
        //绘制下面的数字
    ...
    private void drawOtherData(Canvas canvas,int position,int originalPosition,int type){
        ...
        canvas.drawText(mData.get(position)+"月",x,y,mPaint);
    }
    ...
    public void initData(List<Integer> list,int selected,MonthChangeCallBack mMonthCallBack){
        mData=list;
        ...
        this.mMonthCallBack=mMonthCallBack;
        invalidate();
    }
    ...
    private void moveTop(){//往上滑动
        ...
        mData.remove(0);
        mData.add(tmp);
        mMonthCallBack.monthChange(mData.get(mSelected));
    }
